...
$(function(){
	...
	$('.mnRadio-ra').click(function(){
		$('.checkbox-ra').removeClass('annt-checkbox-checked').attr('disabled','disabled')
		$('#anntDetails').val("")
	...
	//案例分析题展开全文
	$('.box').delegate('.toggleBtn-case','click',function(){
		var _this = $(this);
		if(_this.text()=='展开正文'){
			_this.text('收起正文').addClass('toggleBtn-close');
			_this.closest('ul').find('.question-case-cnt').show();
		}else{
			_this.text('展开正文').removeClass('toggleBtn-close');
			_this.closest('ul').find('.question-case-cnt').hide();
		}
	})
